...
twit.stream('statuses/filter', {follow: twitId, track: person}, function(stream) {
    stream.on('data', function (data) {
        ...
        var twitData = {date_created: data.created_at, user_name: data.user.screen_name, id_str: data.user.id_str, num_followers: data.user.followers_count};
        Tweets.create(twitData);
        console.log("post happend");
    });
});

//, num_entities:Number,  num_tweets:Number, num_retweets: data.retweeted_status.retweet_count, num_mentions: data.retweeted_status.entities.user_mentions,
